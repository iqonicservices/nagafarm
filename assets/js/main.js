...
document.addEventListener('DOMContentLoaded', function () {
  // Content dataset matching your formats
  const formatData = {
    '1': {
      tag: 'RHADASI · 1 KG',
      title: 'Premium Mabele Fine',
      color: '#7D1B36',
      desc: 'A finely milled, premium malted sorghum for home kitchens, heritage recipes and small-batch experimentation.',
      imgSrc: 'assets/img/products/rhadasi-1kg.png', // Updated path
      imgAlt: 'Premium Mabele Fine, 1 kg'
    },
    '3': {
      tag: 'RHADASI · 3 KG',
      title: 'Premium Malted Sorghum',
      color: '#097835',
      desc: 'The everyday brewer’s format: a carefully malted sorghum with character for craft beer, traditional brews and shared tables.',
      imgSrc: 'assets/img/products/rhadasi-3kg.png', // Updated path
      imgAlt: 'Premium Malted Sorghum, 3 kg'
    },
    '20': {
      tag: 'RHADASI · 20 KG',
      title: 'Brewery Supply',
      color: '#153b44',
      desc: 'A working format for craft breweries and serious home brewers. Talk to our team about current availability and delivery.',
      imgSrc: 'assets/img/products/rhadasi-20kg.png', // Updated path
      imgAlt: 'Brewery Supply, 20 kg'
    }
  };

  const buttons = document.querySelectorAll('.rhadasi-format-selector .format-btn');
  const tagEl = document.getElementById('rhadasi-tag');
  const titleEl = document.getElementById('rhadasi-title');
  const descEl = document.getElementById('rhadasi-desc');
  const imgEl = document.getElementById('rhadasi-dynamic-img');

  // Set the initial color if an item is pre-selected on load
  const activeBtn = document.querySelector('.rhadasi-format-selector .format-btn.active');
  if (activeBtn) {
    const initialKg = activeBtn.getAttribute('data-kg');
    if (formatData[initialKg]) {
      titleEl.style.color = formatData[initialKg].color;
    }
  }

  buttons.forEach(button => {
    button.addEventListener('click', function () {
      const selectedKg = this.getAttribute('data-kg');
      const data = formatData[selectedKg];
      ...
      setTimeout(() => {
        ...
        titleEl.style.color = data.color; // Apply the specific color
